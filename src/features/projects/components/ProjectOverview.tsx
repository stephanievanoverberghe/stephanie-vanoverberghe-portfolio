...
export default function ProjectOverview({ project }: { project: Project }) {
    const quickFacts = [
        project.year ? { label: 'Annee', value: String(project.year) } : null,
        project.role?.length ? { label: 'Role', value: project.role.slice(0, 2).join(' · ') } : null,
        project.stack?.length ? { label: 'Stack', value: project.stack.slice(0, 3).join(' · ') } : null,
    ].filter((item): item is { label: string; value: string } => Boolean(item));

    return (
        <section className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr] lg:items-start">
            <article className="surface-card relative overflow-hidden rounded-4xl p-6 sm:p-8">
                <div aria-hidden className="absolute -right-24 -top-24 h-64 w-64 rounded-full bg-(--lilac)/25 blur-3xl" />

                <div className="relative">
                    <p className="text-xs font-bold uppercase tracking-[0.24em] text-(--accent)">Resume</p>
                    ...
                    {project.highlights?.length ? (
                        <div className="mt-7 grid gap-3 sm:grid-cols-2">
                            {project.highlights.slice(0, 4).map((item) => (
                                <div key={item} className="tone-sage-soft rounded-2xl border px-4 py-3">
            ...
            <aside className="space-y-4 lg:sticky lg:top-24">
                <div className="surface-card rounded-4xl p-5">
                    ...
                    <div className="mt-5 grid gap-3">
                        {project.links?.demo ? (
                            <a href={project.links.demo} target="_blank" rel="noopener noreferrer" className="link-card link-card-primary">
                                Voir la demo
                        ...
                        {project.links?.repo ? (
                            <a href={project.links.repo} target="_blank" rel="noopener noreferrer" className="link-card link-card-secondary">
                        ...
                        <Link href="/contact" className="link-card link-card-soft">
                            <span className="inline-flex items-center gap-2">
                ...
                {quickFacts.length ? (
                    <div className="surface-soft rounded-4xl p-5 shadow-(--shadow-card)">
