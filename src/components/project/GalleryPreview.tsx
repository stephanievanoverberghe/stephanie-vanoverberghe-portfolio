        ...
        <section className="relative overflow-hidden rounded-[2.25rem] border border-(--border-soft) bg-(--surface-1) p-5 shadow-(--shadow-card) sm:p-6 lg:p-8">
            ...
            <div className="relative mb-6 flex flex-wrap items-end justify-between gap-4">
                ...
                <button
                    type="button"
                    onClick={() => openAt(0)}
                    className="inline-flex cursor-pointer items-center gap-2 rounded-full border px-4 py-2.5 text-xs font-bold uppercase tracking-[0.14em] text-(--text-strong) transition hover:-translate-y-0.5"
                    style={{
            ...
            <div className="relative grid gap-4 lg:grid-cols-[1.25fr_0.75fr]">
                <button
                    type="button"
                    onClick={() => openAt(0)}
                    className="group relative overflow-hidden rounded-4xl border bg-(--paper) text-left shadow-(--shadow-card) transition duration-500 hover:-translate-y-1 hover:shadow-[0_28px_80px_rgba(18,19,20,0.14)]"
                    style={{ borderColor: 'var(--border-soft)' }}
                    aria-label="Ouvrir la capture principale"
                >
                    <div className="relative aspect-16/10 lg:min-h-130">
                        <Image
                            src={hero.src}
                            alt={hero.alt ?? title ?? 'Capture principale'}
                            fill
                        ...
                        <div
                            ...
                            style={{
                                backgroundImage: 'radial-gradient(1px 1px at 24px 24px, var(--paper) 12%, transparent 13%)',
                            }}
                        />
                        ...
                        <div
                            ...
                            style={{
                                background: 'linear-gradient(to top, rgba(18,19,20,0.52), transparent 58%)',
                            }}
                        />
                        ...
                        <div className="absolute bottom-5 left-5 right-5 flex items-end justify-between gap-5">
                            <div>
                                ...
                                <p className="mt-2 max-w-xl text-lg font-semibold leading-snug text-white">{hero.alt ?? title ?? 'Aperçu du projet'}</p>
                            </div>
                    ...
                    {previews.map((image, index) => {
                        ...
                        return (
                            <button
                                ...
                                className="group relative overflow-hidden rounded-[1.6rem] border bg-(--paper) shadow-(--shadow-card) transition duration-500 hover:-translate-y-1 hover:shadow-[0_18px_50px_rgba(18,19,20,0.12)]"
                                style={{ borderColor: 'var(--border-soft)' }}
                                aria-label={`Ouvrir la capture ${realIndex + 1}`}
                            >
                                <div className="relative aspect-16/10 lg:min-h-29.5">
                                    <Image
                                        src={image.src}
                                    ...
                                    <div className="absolute inset-x-3 bottom-3 flex items-end justify-between gap-3">
                                        <p className="line-clamp-2 text-left text-xs font-semibold leading-5 text-white">{isLast ? `+${hiddenCount} captures` : image.alt}</p>

                                        <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full border text-white opacity-0 transition group-hover:opacity-100">
