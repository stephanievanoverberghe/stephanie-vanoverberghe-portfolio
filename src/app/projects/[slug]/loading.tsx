export default function ProjectLoading() {
    return (
        <article className="container-page py-12 animate-pulse">
            ...
            <div className="mt-6 h-8 w-2/3 rounded bg-(--surface-2)" />
            <div className="mt-3 h-5 w-1/3 rounded bg-(--surface-2)" />

            <div className="mt-5 flex flex-wrap gap-2">
                {Array.from({ length: 6 }).map((_, i) => (
                    <div key={i} className="h-7 w-24 rounded-full border bg-(--surface-1) border-(--border-soft)" />
                ))}
            </div>

            <div className="mt-8 grid gap-6 md:grid-cols-[1.25fr_.75fr]">
                <div className="rounded-2xl border p-6 bg-(--surface-1) border-(--border-soft)">
